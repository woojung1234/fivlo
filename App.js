...
export default function App() {
  ...
  useEffect(() => {
    // Firebase 초기화 및 인증 로직 제거
    // 앱 시작 시 필요한 초기 로직 (예: 로컬 스토리지에서 사용자 세션 확인)을 여기에 추가할 수 있습니다.
    setTimeout(() => {
      setIsAppReady(true); // 앱 준비 완료 (임시)
    }, 1000); // 1초 후 준비 완료 시뮬레이션
  }, []);

  if (!isAppReady) {
    return (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: Colors.primaryBeige }}>
        <Text style={{ fontSize: FontSizes.large, color: Colors.textDark }}>앱 로딩 중...</Text>
      </View>
    );
  }
